SQL> DECLARE
  ...
  6  BEGIN
  7    BEGIN
  8      l_ddl := DBMS_METADATA.GET_DDL('PACKAGE BODY','XXAFW_PROCESS_MONITOR_PKG','WKSP_PENSIONCALC');
 ...
 19      FOR r IN (SELECT text FROM all_source WHERE owner='WKSP_PENSIONCALC' AND name='XXAFW_PROCESS_MONITOR_PKG' ORDER BY line) LOOP
