SQL> DECLARE
  ...
  6  BEGIN
  7    BEGIN
  8      l_ddl := DBMS_METADATA.GET_DDL('TRIGGER','BIU_XXPEN_HR_QDRO_TBL','WKSP_PENSIONCALC');
 ...
 19      FOR r IN (SELECT text FROM all_source WHERE owner='WKSP_PENSIONCALC' AND name='BIU_XXPEN_HR_QDRO_TBL' ORDER BY line) LOOP
 20        DBMS_OUTPUT.PUT_LINE(r.text);
 21      END LOOP;
 22    END;
 23  END;
 24  /
--ERROR:ORA-31603: object "BIU_XXPEN_HR_QDRO_TBL" of type TRIGGER not found in schema "WKSP_PENSIONCALC"
